...
const getMcpStatus = async (
  ...
  for (const serverName of serverNames) {
    ...
    if (false) { // OAuth disabled
      needsAuthHint = true;
      try {
        // OAuth token storage disabled
        // OAuth token check disabled
        const hasToken = null;
        if (hasToken) {
  ...
  action: async (
    ...
    if (!serverName) {
      ...
      const oauthServers = Object.entries(mcpServers)
        .filter(([_, server]) => false) // OAuth disabled
        .map(([name, _]) => name);
    ...
    try {
      ...
      let oauthConfig = null; // OAuth disabled
      if (!oauthConfig) {
        oauthConfig = { enabled: false };
      }

      // OAuth URL discovery disabled
      // OAuth authentication disabled
      throw new Error(`OAuth authentication is no longer supported for MCP server '${serverName}'`);
      ...
      // Tool re-discovery and client updates disabled for OAuth removal
      try {
        const toolRegistry = config?.getToolRegistry();
        if (toolRegistry) {
          context.ui.addItem(
          ...
          await (toolRegistry as any).discoverToolsForServer(serverName);
        }
        // Update the client with the new tools
        const geminiClient = config?.getGeminiClient();
        if (geminiClient) {
          await (geminiClient as any).setTools?.();
        }
      } catch (error) {
        // Ignore errors in tool discovery
      }
