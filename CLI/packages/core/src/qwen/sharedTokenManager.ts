...
// Qwen OAuth import removed
type IQwenOAuth2Client = any;
// Qwen OAuth types removed
type QwenCredentials = any;
type TokenRefreshData = any;
type ErrorData = any;
const isErrorResponse = (data: any): boolean => false;
    ...
    try {
      ...
      if (
        ...
        throw new TokenManagerError(
          ...
          `Failed to access credentials file: ${error instanceof Error ? error.message : String(error)}`,
          error,
        );
      }
  ...
  private async forceFileCheck(qwenClient?: IQwenOAuth2Client): Promise<void> {
    try {
      ...
      if (
        ...
        throw new TokenManagerError(
          TokenError.FILE_ACCESS_ERROR,
          `Failed to access credentials file during refresh: ${error instanceof Error ? error.message : String(error)}`,
          error,
        );
      }
  ...
  private async reloadCredentialsFromFile(
    ...
    try {
      ...
      if (
        ...
        console.warn(`Failed to validate credentials file: ${error instanceof Error ? error.message : String(error)}`);
      }
