...
// MCP OAuth imports removed
// MCPOAuthProvider functionality disabled
...
async function handleAutomaticOAuth(
  ...
  try {
    ...
    throw new Error(`OAuth authentication is no longer supported for MCP server '${mcpServerName}'`);
  ...
  try {
    ...
    if (errorString.includes('401') && hasNetworkTransport(mcpServerConfig)) {
      ...
      const shouldTriggerOAuth =
        mcpServerConfig.httpUrl || false; // OAuth disabled
...
export async function createTransport(
  ...
  let hasOAuthConfig = false; // OAuth disabled
